...
public class StockDAOimpl implements StockDAO {
    ...
    public int save(Stock stock) {
        ...
        sql="INSERT INTO productos(id,codigo,marca,producto,precio,stock,vendidos) VALUES (?,?,?,?,?,?,?)";
    ...
    public int update(Stock stock) {
        ...
        sql="UPDATE productos SET codigo = ?,marca = ?,precio = ?,producto = ?,stock = ?,vendidos = ? WHERE id = ?";
        connection = DBConnection.getConnection();
        try {
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setInt(1,stock.getId());
